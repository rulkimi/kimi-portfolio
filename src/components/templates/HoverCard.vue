...
<template>
  <div @click="goToLink(link)" class="-mx-4 p-4 group cursor-pointer rounded-md hover:bg-slate-800/50 hover:bg-clip-padding hover:backdrop-filter hover:backdrop-blur-sm hover:bg-opacity-10 transition duration-200 hover:shadow-[inset_0_1px_0_0_rgba(148,163,184,0.1)] hover:drop-shadow-lg">
    <div class="grid grid-cols-1 md:grid-cols-12">
      ...
      <!-- Date: Appears at top for small screens, default position for md: and above -->
      <div v-if="date" class="order-1 md:order-none md:col-span-3 text-xs font-medium text-nowrap my-1">{{ date }}</div>
      <!-- Image: Appears at bottom for small screens, default position for md: and above -->
      <img v-else class="order-3 mt-4 md:mt-0 md:order-none md:col-span-3 rounded" :src="imagePath" width="115" :alt="imageAlt">
